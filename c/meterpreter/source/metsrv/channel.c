	...
	DWORD addendLength, ChannelCompletionRoutine *completionRoutine)
{
	...
	dprintf("[CHANNEL] channel_close ENTER. channel=0x%08X id=%u", channel, channel ? channel_get_id(channel) : 0);
	...
	do
	{
		...
		dprintf("[CHANNEL] channel_close. channel=0x%08X id=%u completion=0x%.8x", channel, channel_get_id(channel), completionRoutine);
